...
public class Disk {
    ...
    public ArrayList<Record> getRecordsByBruteForce(int key) {
        ...
        System.out.println(
                "Brute-force data block access is: " + blockAccess + " with " + records.size() + " records found");
    ...
    public ArrayList<Record> getRecordsByBruteForce(int start, int end) {
        ...
        for (int i = 0; i < this.blocks.size(); i++) {
            blockAccess++;
            for (int j = 0; j < getBlockAt(i).curRecords; j++) {
                if ((getBlockAt(i).getRecordAt(j).getNumVotes() >= start)
                        && (getBlockAt(i).getRecordAt(j).getNumVotes() <= end)) {
                    records.add(getBlockAt(i).getRecordAt(j));
                }
            }
        }
    ...
    public ArrayList<Record> deleteRecordsByBruteForce(int key) {
        ...
        System.out.println("Brute-force data block access is: " + blockAccess + " with " + recordsDeleted.size()
                + " deleted records found");
